...
class TestOpenWatermarkCatalog:
    ...
    def test_all_entries_have_required_fields(self):
        required = {
            ...
            "detection_requires",
            "survives_jpeg_q50",
            "korean_tool_adoption",
            "sdk_verdict_without_key",
        }
    ...
    def test_all_unknown_without_key(self):
        ...
        for name, info in OPEN_WATERMARK_CATALOG.items():
            assert info["sdk_verdict_without_key"] == "UNKNOWN", f"{name}: sdk_verdict_without_key 가 UNKNOWN 이 아님"
    ...
    def test_screenshot_kills_all_invisible_watermarks(self):
        ...
        for name, info in OPEN_WATERMARK_CATALOG.items():
            assert info["survives_screenshot"] is False, f"{name}: 스크린샷 후 생존 — 재검증 필요"
